                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                            <div class="flex flex-col sm:flex-row sm:items-start gap-4">
                                <img src="{{ soundcloud_image($campaign_->music?->artwork_url) }}"
                                    alt="{{ $campaign_->music?->title }}" class="w-20 h-20 rounded-lg mx-auto sm:mx-0">
                                <div class="flex-1">
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-3 mb-2">
                                        ...
                                        <span
                                            class="badge badge-soft {{ $campaign_->status_color }} rounded-full">{{ $campaign_->status_label }}</span>
                                    </div>
                                    <div
                                        ...
                                        <div class="flex items-center space-x-1 justify-center sm:justify-start">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                ...
                                                {{ $campaign_->start_date_formatted }}</span>
                                        </div>
                                        <div class="flex items-center space-x-1 justify-center sm:justify-start">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                            ...
                                            </svg><span>{{ __('Expires') }} {{ $campaign_->end_date_formatted }}</span>
                                        </div>
                                    </div>
                                    <div class="mb-4">
                                        <div class="flex items-center justify-between text-sm mb-2"><span
                                                class="text-slate-400">{{ __('Budget used:') }}
                                                {{ $campaign_->credits_spent }} /
                                                {{ $campaign_->budget_credits }}
                                                {{ __('credits') }}</span><span class="text-orange-500 font-medium">
                                                {{ $campaign_->budget_credits > 0 ? number_format(($campaign_->credits_spent / $campaign_->budget_credits) * 100, 2) : 0 }}%</span>
                                        </div>
                                        <div class="w-full bg-orange-600/20 rounded-full h-2">
                                            <div class="bg-gradient-to-r from-orange-500 to-orange-600 h-2 rounded-full transition-all duration-300"
                                                style="width: {{ $campaign_->budget_credits > 0 ? ($campaign_->credits_spent / $campaign_->budget_credits) * 100 : 0 }}%;">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="flex flex-col lg:flex-column sm:items-center sm:space-x-2 gap-2">
                                ...
                                {{-- Add Credits Button --}}
                                <button wire:click="openAddCreditModal({{ $campaign_->id }})"
                                ...
                                <div x-data="{ open: false }"
                                    ...
                                    <div x-show="open" @click.outside="open = false" x-transition x-cloak
                                        ...
                                        <ul class="p-0 text-sm text-gray-700 dark:text-gray-200">
                                            ...
                                            <li>
                                                <button wire:click="openEditCampaignModal({{ $campaign_->id }})"
                                                    @click="open = false"
                                            ...
                                            {{-- cancel Button --}}
                                            <li>
                                                <button wire:click="openCancelWarningModal({{ $campaign_->id }})"
                                                    @click="open = false"
                                ...
                                <button wire:click="openViewDetailsModal({{ $campaign_->id }})"
                        ...
                        <div class="grid grid-cols-3 gap-6">
                            <div class="text-center">
                                <div class="flex items-center justify-center mb-2"><svg
                                        ...
                                        class="text-2xl font-bold   text-black dark:text-gray-100">{{ $campaign_->completed_reposts ?? 0 }}</span>
                                </div>
                            ...
                            <div class="text-center">
                                <div class="flex items-center justify-center mb-2"><svg
                                        ...
                                        class="text-2xl font-bold text-black dark:text-gray-100">{{ $campaign_->playback_count ?? 0 }}</span>
                                </div>
                            ...
                            <div class="text-center">
                                <div class="flex items-center justify-center mb-2"><svg
                                        ...
                                        class="text-2xl font-bold text-black dark:text-gray-100">{{ $campaign_->music?->likes_count ?? 0 }}</span>
                                </div>
