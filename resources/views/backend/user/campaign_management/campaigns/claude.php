<section x-data="{
    ...
    showDeleteWarningModal: @entangle('showDeleteWarningModal').live
}">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start gap-4">
                            ...
                            <div class="flex-1">
                                <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-3 mb-2">
                                    ...
                                    <span class="badge badge-soft {{ $campaign->status_color }} rounded-full">
                                ...
                                <div
                                    ...
                                    <div class="flex items-center space-x-1 justify-center sm:justify-start">
                                        ...
                                        <span>{{ __('Created') }} {{ $campaign->start_date_formatted }}</span>
                                    </div>
                                    <div class="flex items-center space-x-1 justify-center sm:justify-start">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        ...
                                        <span>{{ __('Expires') }} {{ $campaign->end_date_formatted }}</span>
                                    </div>
                                </div>
                                <div class="mb-4">
                                    <div class="flex items-center justify-between text-sm mb-2">
                                        <span class="text-slate-400">{{ __('Budget used:') }}
                                            {{ $campaign->credits_spent }} /
                                            {{ $campaign->budget_credits }}
                                            {{ __('credits') }}
                                        </span>
                                        <span class="text-orange-500 font-medium">
                                            {{ $campaign->budget_credits > 0 ? number_format(($campaign->credits_spent / $campaign->budget_credits) * 100, 2) : 0 }}%
                                        </span>
                                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-6">
                        <div class="text-center">
                            <div class="flex items-center justify-center mb-2">
                                ...
                                <span class="text-2xl font-bold text-black dark:text-gray-100">
                                    {{ $campaign->completed_reposts ?? 0 }}
                                </span>
                            </div>
                    ...
                    <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                        <button type="submit"
                            ...
                            <span wire:loading.remove wire:target="addCreditsToCampaign">
                                {{ $canSubmit ? __('Update Budget') : __('Cannot decrease budget') }}
                            </span>
                ...
                <form wire:submit.prevent="updateCampaign" class="space-y-6">
                    ...
                    <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                        <button type="submit"
                            ...
                            <span wire:loading.remove wire:target="updateCampaign">
                                {{ $canSubmit ? __('Save Changes') : __('Cannot decrease budget') }}
                            </span>
    ...
    <div x-data="{ showDeleteWarningModal: @entangle('showDeleteWarningModal').live }" x-show="showDeleteWarningModal" x-cloak
        x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95"
        x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-200"
        x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
        class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        ...
        <div
            ...
            <div class="p-6 text-center">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
                    {{ __('If you delete this campaign, you will receive a 50% refund of the remaining budget: :amount credits.', ['amount' => number_format($refundAmount)]) }}
                </p>
                <div class="flex justify-center gap-4">
                    <button type="button" x-on:click="showDeleteWarningModal = false"
                    ...
                    <button wire:click="deleteCampaign"
                        ...
                        <span wire:loading.remove wire:target="deleteCampaign">{{ __('Confirm Delete') }}</span>
                        <span wire:loading wire:target="deleteCampaign">{{ __('Deleting...') }}</span>
                    </button>
                </div>
            </div>
        </div>
