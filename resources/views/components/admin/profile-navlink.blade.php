@props(['route' => '', 'active' => '', 'logout' => false, 'label' => ''])
...
@if ($logout)
    ...
    <a href="javascript:void(0)"
        ...
        onclick="event.preventDefault(); document.getElementById('logout-form').submit();">{{ $label }}</a>
...
@else
    <a href="{{ $route }}"
        class="block px-4 py-2 text-text-dark-secondary dark:text-text-white hover:bg-bg-black/10 dark:hover:bg-bg-white/10 transition-colors mx-1 rounded-md">{{ $label }}</a>
@endif
