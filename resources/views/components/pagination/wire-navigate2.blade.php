@php
    ...
    $makeRelativeUrl = function (int $page) use ($keep, $pageParam) {
        // Filter out null/empty values but keep arrays
        $params = array_filter(
            array_merge($keep, [$pageParam => $page]),
            fn($v) => $v !== null && $v !== '' && $v !== [],
        );
...
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between gap-2" x-data>
        ...
        @if ($paginator->onFirstPage())
            <span
                class="inline-flex items-center px-3 py-2 text-sm font-medium rounded-md border border-transparent text-gray-400 dark:text-slate-500 bg-gray-100 dark:bg-slate-800 cursor-not-allowed">
                Previous
            </span>
        @else
            <a href="{{ $makeRelativeUrl($paginator->currentPage() - 1) }}" wire:navigate
                    ...
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <span aria-current="page"
                                class="inline-flex items-center px-3 py-2 text-sm font-semibold rounded-md text-white bg-orange-500 dark:bg-orange-600">
                                {{ $page }}
                            </span>
                        @else
                            <a href="{{ $makeRelativeUrl($page) }}" wire:navigate
                                class="inline-flex items-center px-3 py-2 text-sm font-medium rounded-md border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-slate-200 bg-white dark:bg-slate-900 hover:bg-gray-50 dark:hover:bg-slate-800">
                                {{ $page }}
                            </a>
                        @endif
