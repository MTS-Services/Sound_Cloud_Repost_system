                                ...
                                <div class="flex flex-col sm:flex-row relative items-start sm:items-center justify-between gap-4 mb-4">
                                    ...
                                    <div class="flex items-center justify-between gap-4 w-full">
                                        ...
                                        <div class="relative">
                                            ...
                                            <button
                                                :data-campaign-id="{{ $campaign_->id }}"
                                                x-bind:disabled="!isEligibleForRepost('{{ $campaign_->id }}') || isReposted('{{ $campaign_->id }}')"
                                                ...
                                                :class="{
                                                    'cursor-not-allowed bg-gray-300 dark:bg-gray-600 text-white dark:text-gray-300': !isEligibleForRepost('{{ $campaign_->id }}') && !isReposted('{{ $campaign_->id }}'),
                                                    'cursor-pointer hover:shadow-lg bg-gray-300 dark:bg-gray-600 text-white': isEligibleForRepost('{{ $campaign_->id }}') && !isReposted('{{ $campaign_->id }}'),
                                                    'bg-green-500 text-white cursor-not-allowed': isReposted('{{ $campaign_->id }}'),
                                                ...
                                                <div 
                                                    x-show="!isReposted('{{ $campaign_->id }}')"
    ...
    <script>
        ...
        function trackPlaybackManager() {
            return {
                ...
                init() {
                    // Load persisted tracking data from backend
                    ...
                    // Listen for Livewire updates (pagination, filters)
                    Livewire.hook('morph.updated', () => {
                        // Preserve current tracking state before re-init
                        ...
                        setTimeout(() => {
                            ...
                            // Restore tracking data for campaigns still on page
                            Object.keys(currentTracks).forEach(campaignId => {
                                if (this.tracks[campaignId]) {
                                    // Merge preserved data with new initialization
                                    this.tracks[campaignId] = {
                                        ...this.tracks[campaignId],
                                        actualPlayTime: currentTracks[campaignId].actualPlayTime,
                                        isEligible: currentTracks[campaignId].isEligible,
                                        reposted: currentTracks[campaignId].reposted,
                                    };
                                } else {
                                    // Campaign no longer on page but data should persist
                ...
                loadPersistedTrackingData() {
                    // Get tracking data from Livewire component
                    const trackingData = @json($trackingData ?? []);
                    
                    Object.keys(trackingData).forEach(campaignId => {
                        const data = trackingData[campaignId];
                        
                        if (!this.tracks[campaignId]) {
                            this.tracks[campaignId] = {
                                isPlaying: false,
                                actualPlayTime: parseFloat(data.actual_play_time) || 0,
                ...
                initializeSoundCloudWidgets() {
                    ...
                    playerContainers.forEach(container => {
                        ...
                        // Skip re-binding if widget already exists and is bound
                        ...
                        widget.bind(SC.Widget.Events.PLAY_PROGRESS, (data) => {
                            ...
                            if (track.isPlaying && !track.seekDetected) {
                                ...
                                if (increment > 0 && increment < 2) {
                                    ...
                                    // Sync to backend every 1 second of play time
                                    if (Math.floor(track.actualPlayTime) % 1 === 0) {
                                        this.syncToBackend(campaignId, 'progress');
                                    }
                                }
                            }
                ...
                syncToBackend(campaignId, action) {
                    ...
                    this.$wire.dispatch('updatePlaybackTracking', {
                ...
                clearAllTracking() {
                    ...
                    // DON'T clear tracking data - preserve it
                    // this.tracks = {}; // REMOVED
                    
                    // Clear backend session data
                    this.$wire.dispatch('clearTrackingOnNavigation');
        ...
        window.addEventListener('beforeunload', function() {
            ...
            if (mainElement && mainElement.__x) {
                const trackManager = mainElement.__x.$data;
                if (trackManager && trackManager.clearAllTracking) {
                    trackManager.clearAllTracking();
                }
            }
        });
    </script>
