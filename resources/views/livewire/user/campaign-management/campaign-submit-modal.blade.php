<div x-show="showSubmitModal" x-cloak
    ...
    <div class="w-full max-w-4xl mx-auto rounded-2xl shadow-2xl bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 flex flex-col max-h-[85vh] overflow-hidden">

        <!-- Header -->
        <div class="flex justify-between items-center p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20">
            <div class="flex items-center gap-3">
                <div>
                    @if (app_setting('favicon') && app_setting('favicon_dark'))
                        <img src="{{ storage_url(app_setting('favicon')) }}" alt="{{ config('app.name') }}" class="w-12 dark:hidden" />
                        <img src="{{ storage_url(app_setting('favicon_dark')) }}" alt="{{ config('app.name') }}" class="w-12 hidden dark:block" />
                    @else
                        <img src="{{ asset('assets/favicons/fav icon 1.svg') }}" alt="{{ config('app.name') }}" class="w-12 dark:hidden" />
                        <img src="{{ asset('assets/favicons/fav icon 2 (1).svg') }}" alt="{{ config('app.name') }}" class="w-12 hidden dark:block" />
                    @endif
                </div>
            ...
            @if ($track)
                <div>
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <button @click="showSubmitModal = false; showCampaignsModal = true"
                    ...
                    <div class="p-4 flex items-center space-x-4 dark:bg-slate-700 rounded-xl transition-all duration-200 border border-orange-200">
                        <img src="{{ soundcloud_image($track->artwork_url) }}" alt="Album cover" class="w-12 h-12 rounded">
                        <div>
                            <p class="text-sm text-gray-600 dark:text-white">{{ $track->type }} - {{ $track->author_username }}</p>
            ...
            <form wire:submit.prevent="createCampaign" class="space-y-6">
                ...
                <div class="mt-4">
                    ...
                    <div class="flex items-center justify-center space-x-2 mb-4">
                        <svg class="w-8 h-8 text-orange-500" width="26" height="18" viewBox="0 0 26 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <input type="range" x-init="localCredit = @entangle('credit').defer || 50" x-model="localCredit" min="50" step="10"
                        :max="!proFeatureEnabled ? Math.min(25000, userCreditLimit) : Math.min(25000, userCreditLimit - (localTotalCredit - localCredit))"
                ...
                <div>
                    ...
                    <div class="flex items-start space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-700 dark:text-gray-400">Encourage listeners to comment (2 credits per comment).</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-700 dark:text-gray-400">Motivate users to like your track (2 credits per like).</p>
                    </div>
                </div>
                ...
                <div class="flex flex-col space-y-2">
                    <div class="flex items-start space-x-3">
                        <input type="checkbox" @change="showOptions = !showOptions" wire:model="maxFollowerEnabled"
                            class="mt-1 w-4 h-4 text-orange-500 border-gray-300 rounded focus:ring-orange-500">
                        <span class="text-sm font-medium text-gray-900 dark:text-white">Limit to users with max follower count</span>
                    </div>
                    <div x-show="showOptions" x-transition class="p-3">
                        <div class="flex justify-between items-center gap-4">
                            ...
                            <div class="min-w-[90px] px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-md flex items-center justify-center">
                                <span class="text-sm font-medium text-gray-900 dark:text-white" x-text="localMaxFollower"></span>
                            </div>
                        </div>
                        @error('maxFollower')
                ...
                <div class="border border-gray-200 dark:border-gray-700 bg-gray-200 dark:bg-gray-900 rounded-lg p-4"
                    ...
                    <div class="space-y-3 ml-4">
                        <div class="flex flex-col space-y-2">
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="flex items-center space-x-2">
                                    <span class="text-sm text-gray-700 dark:text-gray-400">Exclude users who repost too often (last 24h)</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col space-y-2">
                            <div class="flex items-start space-x-3">
                                <input type="checkbox" @click="showRepostPerDay = !showRepostPerDay"
                                    ...
                                    :class="momentumEnabled ? 'cursor-pointer' : 'cursor-not-allowed'">
                                <div class="flex items-center space-x-2">
                                    <span class="text-sm text-gray-700 dark:text-gray-400">Limit average repost frequency per day</span>
                                </div>
                            </div>
                                ...
                                <div class="flex justify-between items-center gap-4">
                                    <div class="w-full relative">
                                        <input type="range" x-data :disabled="!momentumEnabled"
                                            ...
                                            max="100" class="w-full h-2 cursor-pointer"
                                            :class="momentumEnabled ? 'cursor-pointer' : 'cursor-not-allowed'">
                                    </div>
                                    <div class="min-w-[80px] px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-md flex items-center justify-center">
                                        <span class="text-sm font-medium text-gray-900 dark:text-white" x-text="localMaxRepostsPerDay"></span>
                                    </div>
                                </div>
                    ...
                    <!-- Genre Selection -->
                    <div class="mt-4">
                        <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Genre Preferences for Sharers</h2>
                        ...
                        <div class="space-y-2 ml-4">
                            ...
                            <div class="flex items-center space-x-2">
                                <input type="radio" name="targetGenre" value="{{ $track?->genre }}"
                                    ...
                                    class="w-4 h-4 text-orange-500 border-gray-300 focus:ring-orange-500"
                                    :class="momentumEnabled ? 'cursor-pointer' : 'cursor-not-allowed'">
                                <span class="text-sm text-gray-700 dark:text-gray-400">Match track genre — {{ $track?->genre }}</span>
                            </div>
                            <div class="space-y-3">
                                <div class="flex items-center space-x-2">
                                    <input type="radio" name="targetGenre"
                                        ...
                                        class="w-4 h-4 text-orange-500 border-gray-300 focus:ring-orange-500"
                                        :class="momentumEnabled ? 'cursor-pointer' : 'cursor-not-allowed'">
                                    <span class="text-sm text-gray-700 dark:text-gray-400">Match one of your profile's chosen genres</span>
                                </div>
                                    ...
                                    @forelse (user()->genres as $genre)
                                        <div class="flex items-center space-x-2">
                                            <input type="radio" name="targetGenre" wire:model='targetGenre'
                                                value="{{ $genre->genre }}"
                                                class="w-4 h-4 text-orange-500 border-gray-300 focus:ring-orange-500">
                                            <span class="text-sm text-gray-700 dark:text-gray-400">{{ $genre->genre }}</span>
                                        </div>
                                    @empty
                                        <div>
                ...
                <div class="pt-4">
                    <button type="submit"
                        class="w-full transition-all duration-300 flex items-center justify-center gap-3 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 font-bold py-2 px-4 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 rounded-lg bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white">
                        <span>
                            <svg class="w-8 h-8 text-white" width="26" height="18" viewBox="0 0 26 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect x="1" y="1" width="24" height="16" rx="3" fill="none" stroke="currentColor" stroke-width="2" />
                                <circle cx="8" cy="9" r="3" fill="none" stroke="currentColor" stroke-width="2" />
                            </svg>
                        </span>
